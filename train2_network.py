...
for i in xrange(n_dataset):
    ...
    try:
	weights='cut_best_weights_{0}_{1}.h5'.format(i,username)
    ...
    checkpoint = ModelCheckpoint('cut_best_weights_{0}_{1}.h5'.format(i, username), monitor='val_acc', verbose=1, save_best_only=True, mode='max')

    reduce_lr = keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience = 5, verbose=1, min_lr=1e-10)
    ...
    model.fit_generator(datagen.flow(X_train, Y_train,
	    ...
	    callbacks=[history, checkpoint, reduce_lr, early_stop])
